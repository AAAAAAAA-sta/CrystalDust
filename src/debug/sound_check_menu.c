...
static bool8 Task_ProcessSoundCheckMenuInput(u8 taskId) // sub_080E8688
{
    ...
    else if (JOY_REPEAT(DPAD_LEFT))
    {
        ...
        else
        {
            if (gTasks[taskId].tBgmIndex > 0)
                ...
                gTasks[taskId].tBgmIndex = (MUS_GSC_ROUTE11 - (MUS_LITTLEROOT_TEST - 1));
        }
        return TRUE;
    }
    else if (JOY_REPEAT(DPAD_RIGHT))
    {
        ...
        {
            if (gTasks[taskId].tBgmIndex < (MUS_GSC_ROUTE11 - (MUS_LITTLEROOT_TEST - 1)))
                gTasks[taskId].tBgmIndex++;
            else
                gTasks[taskId].tBgmIndex = 0;
        }
...
#define SOUND_LIST_BGM \
    ...
    X(MUS_GSC_CELADON, "MUS-GSC-CELADON") \
